...
La variable `var` est généralement celui qui peut prendre beaucoup d’espace et qui peut être nettoyé facilement.

Adobe Commerce stocke tous les fichiers journaux dans `var` répertoire . De nouveaux fichiers journaux sont créés et les anciens sont archivés quotidiennement. Mais si le nombre d’erreurs générées continue à augmenter, les fichiers journaux prennent de plus en plus d’espace.

Les fichiers d’importation/exportation personnalisés sont également stockés dans la variable `var` et prenez de l’espace si leur nombre augmente.
...
* Nettoyez la variable `var` répertoire .
* Configuration d’une tâche cron pour le suivi de la taille de la variable `var` et nettoyez-le.
* Allouez plus d’espace disque, si vous n’en avez pas. (Voir la section ci-dessous pour plus d’informations sur la façon de vérifier quelle est votre limite d’espace.)
   * Pour le plan de démarrage, tous les environnements et les environnements d’intégration ProPlan, vous pouvez allouer de l’espace disque si vous n’en avez pas utilisé, comme décrit dans la section [Gérer l’espace disque : allocation de l’espace disque](https://devdocs.magento.com/guides/v2.3/cloud/project/manage-disk-space.html#application-disk-space).
...
1. Sur le **[!UICONTROL All projects]** dans le tableau de bord, sélectionnez le projet approprié. Dans le coin gauche, vous pouvez voir la disponibilité de l’espace disque.
