...
1. Créez un panier vide via GraphQL (modifiez l’emplacement en fonction de vos besoins). [!DNL URL]) :
...
1. Exécutez deux demandes simultanées pour ajouter le même produit au panier (modification). *cartID* et emplacement des deux requêtes) :
...
1. Obtenir le panier pour afficher les éléments (modifier *cartId* et emplacement) :
...
Élément répertorié une seule fois avec [!UICONTROL qty] = 2.
