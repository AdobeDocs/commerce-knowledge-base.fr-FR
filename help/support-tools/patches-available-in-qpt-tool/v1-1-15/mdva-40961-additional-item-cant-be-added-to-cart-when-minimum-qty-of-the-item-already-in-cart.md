...
Le correctif MDVA-40961 corrige le problème lorsqu’un article supplémentaire ne peut pas être ajouté au panier alors que la quantité minimale de l’article est déjà dans le panier. Ce correctif est disponible lorsque la variable [Outil Correctifs de qualité (QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) La version 1.1.15 est installée. L’ID de correctif est MDVA-40961. Veuillez noter que le problème doit être corrigé dans Adobe Commerce 2.4.5.
...
1. Définir un produit simple pour avoir plusieurs **Quantité minimale autorisée dans le panier** (par exemple, deux).
...
Le message d’erreur suivant s’affiche : *Le moins que vous puissiez acheter est 2*.
