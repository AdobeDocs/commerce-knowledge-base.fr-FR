...
Le correctif MDVA-44188 corrige le problème où les emails ne sont pas envoyés aux ID d’email contenant `.-`. Ce correctif est disponible lorsque la variable [Outil Correctifs de qualité (QPT)](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) La version 1.1.13 est installée. L’ID de correctif est MDVA-44188. Veuillez noter que le problème doit être corrigé dans Adobe Commerce 2.4.5.
...
1. Enregistrement d’un client avec une adresse électronique contenant `.-`. Par exemple : `.-foo@example.com`.
1. passé une commande ;

<u>Résultats attendus</u>:

Utilisateur avec l’ID d’adresse électronique contenant `.-` reçoit des emails comme vous le faites habituellement.

<u>Résultats réels</u>:

L’e-mail n’est pas envoyé à l’e-mail contenant `.-`. L’erreur suivante s’affiche dans les journaux d’erreurs : *Impossible d’ajouter une adresse électronique non valide à la file d’attente de messagerie*.
