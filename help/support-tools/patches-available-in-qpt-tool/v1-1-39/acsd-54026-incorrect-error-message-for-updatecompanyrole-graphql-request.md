...
Le correctif ACSD-54026 corrige le problème en raison duquel un message d’erreur incorrect s’affiche pour un `updateCompanyRole` Requête GraphQL pour un utilisateur non autorisé. Ce correctif est disponible lorsque la variable [!DNL Quality Patches Tool (QPT)] La version 1.1.39 est installée. L’ID de correctif est ACSD-54026. Veuillez noter que le problème doit être corrigé dans Adobe Commerce 2.4.7.
...
Obtention d’un message d’erreur incorrect pour une `updateCompanyRole` Requête GraphQL pour un utilisateur non autorisé.

<u>Étapes à reproduire</u>:

1. Activez la société B2B dans la configuration.
1. Créez une société.
1. Envoyer un `updateCompanyRole` Requête GraphQL sans transmission d’un jeton porteur ou avec un jeton porteur non valide.
...
Le message d’erreur suivant s’affiche : *Le client n’est pas un utilisateur de l’entreprise.*
