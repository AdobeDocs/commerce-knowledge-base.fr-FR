...
1. Exécuter `show processlist` dans MySQL.
...
`GET_LOCK` doit être exécutée immédiatement.

<u>Résultats réels</u>:

Multiple `GET_LOCK` les requêtes sont bloquées pendant 10 secondes au maximum.
