...
Le correctif ACSD-55031 corrige le problème en raison duquel la variable `Type "mixed" cannot be nullable` lors de la compilation après l’installation d’une extension personnalisée. Ce correctif est disponible lorsque la variable [[!DNL Quality Patches Tool (QPT)]](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) La version 1.1.40 est installée. L’ID de correctif est ACSD-55031. Veuillez noter que le problème a été corrigé dans Adobe Commerce 2.4.6.
...
La variable `Type "mixed" cannot be nullable` s’affiche pendant la compilation.
...
La variable `var/log/system.log` contient l’erreur :
